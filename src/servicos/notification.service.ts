...
import { EntradaFila, Reserva, SituacaoFila, SituacaoReserva } from '../dominio/entidades.js';
  ...
  async processarNotificacoesPendentes() {
    ...
    for (const item of pendentes) {
      ...
      try {
        ...
        this.logger.error(`Falha ao notificar aluno ${item.aluno.id} sobre reserva do livro ${item.livro.id}`);
      }
    }
  }
}
