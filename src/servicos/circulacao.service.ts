...
@Injectable()
export class CirculacaoService implements OnModuleInit {
  constructor(
  ...
  async onModuleInit() {
    // Normaliza registros criados antes da remoção do estado intermediário.
    await this.reservas
      .createQueryBuilder()
      .update()
      .set({ situacao: SituacaoReserva.SOLICITADA })
  ...
  async liberarExemplar(gestor: EntityManager, livro: Livro) {
    ...
    proximo.notificadoEm = undefined;
  ...
  async cancelarReserva(id: string, alunoId: string) {
    return this.banco.transaction(async (gestor) => {
      ...
      if (reserva.situacao !== SituacaoReserva.SOLICITADA)
        throw new ConflictException(
  ...
  async retirar(id: string) {
    ...
    if (reserva.situacao !== SituacaoReserva.SOLICITADA)
      throw new ConflictException(
        'A reserva deve estar solicitada para registrar a retirada.',
      );
