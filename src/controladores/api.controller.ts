...
export class AutenticacaoController {
  ...
  async concluirPrimeiroAcesso(
    ...
    await this.usuarios.concluirPrimeiroAcesso(
      u,
      d.currentPassword,
      d.newPassword,
    );
...
export class LivrosController {
  ...
  @UseGuards(SomenteAdministrador)
  @UseInterceptors(
    FileInterceptor('cover', { limits: { fileSize: 2 * 1024 * 1024 } }),
  )
  async enviarCapa(@Param('id', ParseUUIDPipe) id: string, @UploadedFile() arquivo: any) {
    if (!arquivo)
      throw new BadRequestException('Selecione uma imagem para a capa.');
    ...
    if (!extensao)
      throw new BadRequestException(
        'A capa deve ser uma imagem JPEG, PNG ou WebP.',
      );
